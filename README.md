...
专门为 **GPT Image 2 / `gpt-image-2`** 做的本地图片生成工具。现在对外已经收口成一套统一主应用，客户前台提供首页愿景、影像定制、模板选用、智能生图、作品管理、登录注册和我的积分；后台管理不在客户前台展示，后续独立内网端口给工作人员接单、制作和上传成果。
...
- 一级菜单：`首页` / `影像定制` / `图片创作` / `我的作品` / `注册` / `我的积分`
- 当前客户前台采用 6 个业务板块，不是界面排版规则；前端可以按响应式设计横排或换行。后续如果业务设计要调整板块数量、命名或顺序，先改文档确认，再改代码。
- `首页`：讲清“修复旧照、亲人重逢、爱人合影、正式交付”的产品愿景，并把客户导向影像定制或图片创作。
- `影像定制`：`样片集` / `提交资料` / `制作状态` / `查看成片`
- `影像定制` 只面向客户注册用户；客户不处理三视图、人物确认、像不像审核、模板生成和成果上传。
- `样片集`：先选模板大类，再从左侧子场景切换样片，右侧看大图并可放大。
- `提交资料`：按人上传照片，勾选影像组合和风格方向。
- 后台制作平台单独给管理员和工作人员使用，不并入客户前台；后续通过独立内网端口访问。
- `图片创作`：`模板库` / `智能生图`
- `我的作品`：`全部作品` / `已删除`
- `注册`：默认创建账号，填写用户名、邮箱、邮箱回执码和密码；已有账号可切到邮箱登录
- `我的积分`：`我的积分` / `买积分`

API Key、接口模式和模型默认按系统已接好的通道处理，不作为客户前台的显式操作。没有账号的用户，可以通过主应用里的 `注册` 面板填写用户名、邮箱、邮箱回执码和密码完成站内注册。如果要启用 PostgreSQL 存储，先配置 `.env` 里的 `APP_STORAGE_BACKEND=postgres` 和 `POSTGRES_*`，再执行：
...
- `首页`
  - 客户第一屏，负责讲清旧照修复、亲人纪念合影和合影遗憾的产品愿景。
...
- `planning/01-架构与规范/10-当前实现文档.md`：当前 Node 主应用、接口、存储、目录和验证口径。
- `planning/02-产品与交付/03-个人AI摄影棚产品设计.md`：个人 AI 摄影棚的产品定位、页面架构和冻结口径。
- `planning/02-产品与交付/10-宣传理念与对外表达规范.md`：宣传主张、客户价值和对外文案边界。
